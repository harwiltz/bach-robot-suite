...
class RocketLandingScene(Scene):
    def reset(self):
        Scene.reset(self)
        plane_path = os.join(pybullet_data.getDataPath(), "plane.urdf")
        self.plane = self.p.loadURDF(plane_path, [0, 0, -4])
        water_path = os.join(brs_data.getDataPath(), "water.urdf")
        water_ori = [np.pi/2, 0, 0]
        self.water = self.p.loadURDF(water_path, [0, 0, -1], water_ori)
        self.p.changeVisualShape(self.water, -1, rgbaColor=[0.2, 0.2, 0.6, 0.7])
        pad_path = os.join(brs_data.getDataPath(), "martlet9", "landingpad.urdf")
